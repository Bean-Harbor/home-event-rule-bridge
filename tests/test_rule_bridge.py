...
class RuleBridgeTests(unittest.TestCase):
    ...
    def test_help_and_status_are_human_readable(self) -> None:
        ...
        self.assertIn("First run:", help_reply.text)
        self.assertIn("devices", help_reply.text)
        self.assertIn("find <thing>", help_reply.text)
        self.assertIn("show yaml", help_reply.text)
        self.assertIn("confirm", help_reply.text)
